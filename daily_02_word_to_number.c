...
int sum_intervals(const struct interval *v, size_t n)
{
    ...
    for (int i = 0; i < n - 1; i++)
    {
        if (seconds[i] < firsts[i + 1])
            sum = firsts[i + 1] - seconds[i];
    }
        ...
        if (string[i] == ' ' || string[i] == '-' || string[i] == '\0')
        {
            ...
            if (buffer_size > 0)
            {
                ...
                if (strcmp(t, "hundred") == 0)
                    tk->a[tokens_size] = 100;
                if (strcmp(t, "thousand") == 0)
                    tk->a[tokens_size] = 1000;
                if (strcmp(t, "million") == 0)
                    tk->a[tokens_size] = 1000 * 1000;
                if (strcmp(t, "and") == 0)
                {
                    buffer_size = 0;
                    buffer[buffer_size] = '\0';
...
int main()
{
    ...
    // 130
    // sum_intervals((const struct interval[]){{231, 255}, {127, 132}, {-73, -65}, {-231, -229}, {-228, -205}, {54, 55}, {198, 199}, {148, 171}, {-50, -47}, {213, 218}, {26, 31}, {62, 68}, {-175, -174}, {-146, -141}, {-5, 0}, {-117, -112}, {-100, -93}, {114, 115}}, 18);
    ...
    Token *t = tokenize_word("two million seven hundred eighty-three thousand nine hundred and nineteen");
    ...
    // char str[] = "seven hundred eighty-three thousand nine hundred and nineteen";
    // char *pch = strtok(str, " ,.-");
    // while (pch != NULL)
    // {
    //   printf("%s\n", pch);
